...
\begin{frame}[fragile]{Root Locus Plot with Verification}
...
# Given points ±j√11
y = np.sqrt(11)
plt.plot(0, y, 'ro')    # +j√11
plt.plot(0, -y, 'ro')   # -j√11
plt.title("Root Locus with Given Points")
plt.grid()
plt.show()
\end{lstlisting}
...
\begin{frame}[fragile]{Verification of $s = \pm j\sqrt{11}$}
...
% \begin{frame}[fragile]{Step Response Plot (Python Code)}
...
\begin{center}
\includegraphics[width=0.8\linewidth]{final root locus.png}
\end{center}
...
\begin{center}
\includegraphics[width=0.8\linewidth]{root_locus.png}
\end{center}
